...
* **graph_start** -- Optional field that holds settings for special event of starting the graph (little different from usual event handling settings). Start case also has a different order of handling than the rest of the node. An active version is created very early so functions from regular event handling can be reused here. Any setup callbacks are run first to setup the session since that would regularly be handled during a transition and the starting node may depend on a session. Filters are run after so any filters that depend on session data don't break and start can have similar filters to transition handling
