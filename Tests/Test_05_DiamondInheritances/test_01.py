...
def test_parsed_fields():
    parsed_fields_one = [field["name"] for field in type_cache["ChildOne"].ChildOneGraphNode.CLASS_FIELDS[1]]
    assert "CN1" in parsed_fields_one
    assert "CN2" not in parsed_fields_one
    assert "GC" not in parsed_fields_one
    parsed_fields_two = [field["name"] for field in type_cache["ChildTwo"].ChildTwoGraphNode.CLASS_FIELDS[1]]
    assert "CN1" not in parsed_fields_two
    assert "CN2" in parsed_fields_two
    assert "GC" not in parsed_fields_two
    parsed_fields_three = [field["name"] for field in type_cache["Grandchild"].GrandchildGraphNode.CLASS_FIELDS[1]]
...
def test_schema():
    ...
    parsed_schema_one = type_cache["ChildOne"].ChildOneGraphNode.PARSED_SCHEMA[1]
    assert len(parsed_schema_one["allOf"]) == 2
    parsed_schema_two = type_cache["ChildTwo"].ChildTwoGraphNode.PARSED_SCHEMA[1]
    assert len(parsed_schema_two["allOf"]) == 2
    parsed_schema_three = type_cache["Grandchild"].GrandchildGraphNode.PARSED_SCHEMA[1]
    assert len(parsed_schema_three["allOf"]) == 13
