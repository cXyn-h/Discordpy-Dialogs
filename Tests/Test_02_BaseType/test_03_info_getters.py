...
def test_parsed_fields_copy():
    if "PARSED_FIELDS" in vars(BaseType.BaseGraphNode).keys():
        BaseType.BaseGraphNode.clear_caches()
    fields = BaseType.BaseGraphNode.get_node_fields()
    assert fields is not BaseType.BaseGraphNode.PARSED_FIELDS
    fields2 = BaseType.BaseGraphNode.get_node_fields()
    assert fields2 is not BaseType.BaseGraphNode.PARSED_FIELDS
